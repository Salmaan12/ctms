...
session_start();
    ...
    if (!isset($_SESSION['is_admin'])) {
        ...
        if ($now > $_SESSION['expire']) {
          ...
          if(isset($_GET["delid"]))
{
	$delres2 = mysqli_query($con, "DELETE FROM fixtures where fixtureid='$_GET[delid]'");
		if(!$delres2)
			{
...
if($_SESSION['setvar'] == $_POST['setvar'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$insres = mysqli_query($con, "UPDATE  fixtures SET  levelid='$_POST[levelid]',level='$_POST[level]',tournamentid =  '$_GET[tournamentid]',teamid1 =  '$_POST[team1]',teamid2 =  '$_POST[team2]',venue =  '$_POST[venue]',fixdatetime = '$fixdatetime', matchtype =  '$_POST[type]' WHERE  fixtureid ='$_GET[editid]'");
		...
		{
			$insres = mysqli_query($con, "INSERT INTO fixtures (levelid,level,tournamentid,teamid1,teamid2,venue,fixdatetime,matchtype) values ('$_POST[levelid]','$_POST[level]','$_GET[tournamentid]','$_POST[team1]','$_POST[team2]','$_POST[venue]','$fixdatetime','$_POST[type]')");
			echo mysqli_error($con);
			if(!$insres)
			{
				$qresulti =  1;
...
$selectquery = mysqli_query($con, "SELECT * FROM fixtures where fixtureid='$_GET[editid]'");
...
<div class="wrapper row3">
    <div id="container">
        <div id="portfolio" class="three_quarter">
			...
			$sqlquery  = mysqli_query($con,$sqlteams);
		...
		?>
                ...
                <tr>
                    ...
                    <td><?php
  /*
    $qres = mysqli_query($con, "select tournaments.*,tournamentteams.*,teams.*  from tournaments INNER JOIN tournamentteams INNER JOIN teams ON tournaments.tournamentid=tournamentteams.tournamentid AND tournamentteams.teamid= teams.teamid where tournamentteams.tournamentid='$_GET[tournamentteams]' ");*/
$qresteams = mysqli_query($con, "SELECT DISTINCT teamid FROM tournamentteams WHERE tournamentid =  '$_GET[tournamentid]'");
	while($arrrecteams = mysqli_fetch_array($qresteams))
	{

$qres = mysqli_query($con, "select tournaments.*,tournamentteams.*,teams.* from tournaments INNER JOIN tournamentteams INNER JOIN teams ON tournaments.tournamentid=tournamentteams.tournamentid AND tournamentteams.teamid= teams.teamid where tournamentteams.teamid='$arrrecteams[teamid]' ");
...
?>
            <?php
  $qtournamentlevel = mysqli_query($con, "SELECT DISTINCT levelid,level FROM fixtures where  tournamentid =  '$_GET[tournamentid]'");
	while($arrtournamentlevel = mysqli_fetch_array($qtournamentlevel))
	{
?>
            <h2 align="center"><u>Level
                    <?php echo $arrtournamentlevel['levelid']. " - " . $arrtournamentlevel['level'] ; ?> </u></h2>
...
?>
	...
	?>
                    <td><input type="date" name="fixdate" value="<?php echo $mysqldate; ?>" /><br /><input type="time"
                            name="fixtime" value="<?php echo $mysqltime; ?>" /></td>
                    <td><select name="type">
                            <option value="">Select</option>
                            <?php
...
?>
	...
	?>
                            <input type="date" name="fixdate" value="<?php echo $mysqldate; ?>" /><input type="time"
                                name="fixtime" value="<?php echo $mysqltime; ?>" />
                        </td>
                        <td><select name="type">
                                <option value="">Select</option>
                                <?php
...
$("#fixture").click(function() {
    $.post($("#fixtureForm").attr("action"), $("#fixtureForm :input").serializeArray(), function(
        info) {
        $("#result").html(info);
    });
});
